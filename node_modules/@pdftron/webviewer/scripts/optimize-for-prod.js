...
// `optimize-script-helper.mjs` is native ESM (also imported by Vitest browser tests). It is loaded via dynamic `import()` inside the async IIFE below.
...
/**
 * Returns the PDF versions that need their workers split into chunks.
 * @param {'y' | 'n'} keepFullAndLean
 * @param {'y' | 'n'} useFullAPI
 * @returns {string[]}
 */
const getPDFVersionsToSplit = (keepFullAndLean, useFullAPI) => {
  if (keepFullAndLean === 'y') {
    return ['full', 'lean'];
  }
  return useFullAPI === 'y' ? ['full'] : ['lean'];
};
...
if (require.main === module) {
  ...
  if (mainOptions.command === 'auto') {
    const mergeDefinitions = [
      ...
      { name: 'keepFullAndLean', type: Boolean, defaultValue: false },
  ...
  (async () => {
    const { optimizeFileManagement } = await import('./optimize-script-helper.mjs');
    try {
    ...
    console.log(CYAN, `\nPress CMD/CTRL + C at any time to safely cancel this process. If you are unsure of any answer, ${UNDER}please clarify${RESET}${CYAN} before answering them.`, RESET);
    ...
    const schema = {
      properties: {
        ...
        keepFullAndLean: {
          description: `Do you want to keep both the full and lean versions of PDFNet.js in your build? Keeping both versions will increase the size of your build, but will allow you to switch between the two versions. [y/n]${RESET}`,
          ...globalPrompt,
          ask: () => !optimizeOptions.hasOwnProperty('keepFullAndLean'),
        },
        useFullAPI: {
          description: `Do you need the full PDF API? See ${CYAN}https://docs.apryse.com/documentation/web/guides/optimize-lib-folder${RESET}${DIM} for more info (most users don't need this option). [y/n]${RESET}`,
          ...globalPrompt,
          ask: () => {
            return checkRequiredPrompts('keepFullAndLean', 'n') && !optimizeOptions.hasOwnProperty('useFullAPI');
          },
        },
        pdfnetProd: {
    ...
    prompt.get(schema, (err, result) => {
      ...
      if (mainOptions.command === 'auto') {
        ...
        keepFullAndLean = optimizeOptions.keepFullAndLean ? 'y' : 'n';
      ...
      filesToRelocate.push(
        ...
        [path.resolve(__dirname, `../${wvFolder}/core/pdf/PDFNetLean.js`), `${resourceDir}/${wvFolder}/core/pdf/`, true],
      ...
      prompt.get({
        ...
        if (result.delete === 'y') {
          ...
          if (salesforceSupport === 'y') {
            ...
            const pdfVersionsToSplit = getPDFVersionsToSplit(keepFullAndLean, useFullAPI);

            pdfVersionsToSplit.forEach((pdfVersion) => {
              const chunkWorkerDirectoryPath = splitWorkerFiles(
                path.resolve(__dirname, `../${wvFolder}/core/pdf/${pdfVersion}/optimized/PDFNetCWasm.br.wasm`),
                2,
                resourceDir,
                pdfVersion
              );
